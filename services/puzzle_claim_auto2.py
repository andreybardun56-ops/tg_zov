...
def init_cookie_jar(cookies: Dict[str, str]) -> aiohttp.CookieJar:
    jar = aiohttp.CookieJar(unsafe=True)
    if cookies:
        jar.update_cookies(cookies, response_url=URL(EVENT_PAGE))
    return jar


def cookies_from_jar(jar: aiohttp.CookieJar) -> Dict[str, str]:
    filtered = jar.filter_cookies(URL(EVENT_PAGE))
    return {k: v.value for k, v in filtered.items()}


async def warmup_event_page(session: aiohttp.ClientSession, profile: Dict[str, Any]):
    try:
        headers = build_navigation_headers(profile)
        async with session.get(EVENT_PAGE, headers=headers) as resp:
            await resp.text()
            logger.info("[PUZZLE_CLAIM2] 🌐 Теплый заход на страницу puzzle2: %s", resp.status)
    except ClientError as e:
        logger.warning("[PUZZLE_CLAIM2] ⚠️ Ошибка прогрева puzzle2: %s", e)


async def perform_claim_request(
...
async def claim_puzzle(tg_user_id: str, target_iggid: str, puzzle_num: int, bot, msg=None) -> None:
  ...
  #  )
    ...
    try:
        ...
        cookie_jar = init_cookie_jar(acc_cookies)
        ...
        async with aiohttp.ClientSession(
            ...
            await warmup_event_page(session, profile)
            await asyncio.sleep(0.6 + 0.6 * random.random())

            fresh_map = cookies_from_jar(session.cookie_jar)
            if fresh_map:
                cookies_db.setdefault(tg_user_id, {})[str(target_iggid)] = fresh_map
                save_cookies_file(cookies_db)

            status, text = await perform_claim_request(session, profile, donor_iggid, puzzle_num)
...
async def check_puzzle2_active(user_id: str) -> bool:
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            headers = build_navigation_headers(profile)
            async with session.get(EVENT_URL, headers=headers, allow_redirects=True) as resp:
                html = await resp.text()
                current_url = str(resp.url).lower()
                ...
                is_active = any(marker in html_lower for marker in active_markers)
